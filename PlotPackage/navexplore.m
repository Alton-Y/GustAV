...
%     x = plot(FMT.NTUN.TimeS,FMT.NTUN.XT,'--r');
    x = plot(FMT.GPS.TimeS,FMT.GPS.GCrs,'.');
...
% yyaxis left
hold on
arsp = plot(FMT.CTUN.TimeS,FMT.CTUN.Aspd,'.');
arsp_g = plot(FMT.GPS.TimeS,FMT.GPS.Spd,'.');
...
accel=smooth(FMT.IMU.AccX,1,'lowess') -...
    (9.80665.*sind( interp1(FMT.ATT.TimeS,smooth(FMT.ATT.Pitch,1,'lowess'),FMT.IMU.TimeS)));

aplot=plot(FMT.IMU.TimeS,accel,'.');
